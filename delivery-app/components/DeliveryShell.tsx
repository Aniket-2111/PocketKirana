...
import { locationManager } from '../lib/deviceLocation';
...
export default function DeliveryShell({
  ...
  const [locState, setLocState] = useState(() => locationManager.getState());

  useEffect(() => {
    const unsub = locationManager.subscribe((s) => {
      setLocState(s);
    });
    return () => unsub();
  }, []);
  ...
  const handleToggleDuty = async () => {
    if (!isPartnerOnline) {
      const currentLoc = await locationManager.checkState({ forceFresh: true });
      if (currentLoc.status !== 'READY') {
        if (currentLoc.permission === 'DENIED') {
          showToast('Location permission is required to go Online.', 'error');
          locationManager.openAppSettings();
          return;
        }
        if (currentLoc.gps === 'DISABLED' || currentLoc.status === 'GPS_DISABLED') {
          showToast('Device GPS must be ON to go Online.', 'error');
          locationManager.openLocationSettings();
          return;
        }
        const granted = await locationManager.requestPermission();
        if (!granted) {
          showToast('Location access is required for delivery shifts.', 'error');
          return;
        }
      }
    }
  ...
  const isGpsDisabledWhileOnline = isPartnerOnline && (locState.gps === 'DISABLED' || locState.status === 'GPS_DISABLED');

  return (
    <LocationPermissionGuard>
      <div className={`bg-[#F3F5F7] text-slate-900 flex flex-col font-sans selection:bg-emerald-600 selection:text-white ${fixedScreen
          ...
          <div className="max-w-md mx-auto flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3 min-w-0">
              <div className="relative w-10 h-10 rounded-full bg-[#0F532B] text-white flex items-center justify-center shrink-0">
                ...
                <span className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white ${isPartnerOnline ? 'bg-emerald-400' : 'bg-slate-400'
                  }`} />
              </div>
            ...
            <button
              ...
              type="button"
              className={`py-1.5 px-3.5 rounded-full border flex items-center gap-1.5 text-xs font-bold transition-all cursor-pointer active:scale-95 ${isPartnerOnline
                  ? 'bg-emerald-50 border-emerald-300 text-[#0F532B]'
                  : 'bg-slate-100 border-slate-300 text-slate-600'
                }`}
            >
              <span className={`w-2 h-2 rounded-full ${isPartnerOnline ? 'bg-[#0F532B] animate-pulse' : 'bg-slate-400'
                }`} />
        ...
        {/* ── GPS DISABLED WHILE ONLINE WARNING BANNER ── */}
        {isGpsDisabledWhileOnline && (
          <div className="bg-amber-500 text-white px-4 py-2 text-xs font-bold text-center flex items-center justify-between gap-2 shadow-sm">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-white animate-ping" />
              <span>GPS is turned off — tracking paused</span>
            </div>
            <button
              onClick={() => locationManager.openLocationSettings()}
              className="bg-white text-amber-900 font-black px-2.5 py-1 rounded-lg text-[10px] cursor-pointer"
            >
              Turn On
            </button>
          </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`relative flex flex-col items-center justify-center gap-0.5 pt-0.5 transition-colors cursor-pointer ${isActive ? 'text-[#0F532B]' : 'text-slate-400'
                    }`}
                >
                  {isActive && (
