...
export const ServiceBenefits: React.FC = () => {
  ...
  return (
    <section className="my-10 bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-xs">
        ...
        {benefits.map((item, idx) => {
          ...
          return (
            <div key={idx} className="flex items-start gap-4 text-left">
              <div
                className={`w-12 h-12 rounded-2xl ${item.bg} ${item.iconColor} ${item.border} border flex items-center justify-center shrink-0 shadow-2xs`}
              >
                <Icon className="w-6 h-6" />
              </div>
              <div className="space-y-1">
                <h4 className="text-sm font-black text-[#075C3C] leading-snug">
                  {item.title}
                </h4>
