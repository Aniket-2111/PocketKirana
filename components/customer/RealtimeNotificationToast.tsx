...
// Pleasant harmonic two-tone audio chime using Web Audio API (Zero latency, no asset 404s)
function playHarmonicChime() {
  try {
    ...
    // Tone 2: B5 (987.77 Hz) - 100ms offset
...
export function RealtimeNotificationToast() {
  ...
  const displayNextToast = useCallback(() => {
    ...
    // Auto-dismiss in 6000ms with progress bar
  ...
  // Global event listener for custom triggered notifications
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (user: User | null) => {
      ...
      if (!user) {
        // Fallback: check guest / anonymous uid in localStorage
        const guestUid = localStorage.getItem('pk_guest_uid') || localStorage.getItem('user_id');
        if (!guestUid) return;
      }
        ...
        unsubscribeFirestore = onSnapshot(
          ...
          (snapshot) => {
            if (initialLoadRef.current) {
              // Pre-populate shown IDs on initial page load to avoid toast spam
